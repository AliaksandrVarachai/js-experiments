...
const { elderAgeSimple, elderAge } = require('./become-immortal');

const { assert } = chai;

const testData = [
//   { args: [1,4,0,100000], ans: 6 },
  { args: [15,7,1,100], ans: 5 },
...
describe('execution', () => {
  ...
  xit('Should run set of optimized tests', () => {
    const actualItems = testData.map(({ args }) => elderAge(...args));
    const expectedItems = testData.map(({ ans }) => ans);
    assert.sameOrderedMembers(actualItems, expectedItems);
  });
});
